            ...
            <div className="shopping-cart">
              <div className="group-img-and-name">
                ...
                <div className="group-name-denom-qty">
                  ...
                  <p className="denomination">
                    Denomination: ${Number(itm.giftCardDenomination).toFixed(2)}
                  </p>
                  <p className="sender-fee">
                    Sender Fee: $
                    {(Number(itm.fee) * Number(itm.quantity)).toFixed(2)}
                  </p>
                  <div className="group-qty-and-remove">
                    <p>
                      <select
                        onChange={(event) =>
                          cart.cartQuantityChangeHandler(event, itm)
                        }
                        name="quantity"
                        id="quantity"
                        required
                      >
                        {selectQty().map((number) =>
                          number === Number(itm.quantity) ? (
                            <option value={itm.quantity} selected>
                              {itm.quantity}
                            </option>
                          ) : (
                    ...
                    <Button
                      variant="link"
                      onClick={() =>
                        cart.deleteFromCart(itm.id, itm.giftCardDenomination)
                      }
                    >
                      Remove
                    </Button>
              ...
              <div className="group-price">
                ...
                <p className="price">
                  $
                  {(
                    itm.giftCardDenomination * itm.quantity +
                    itm.fee * itm.quantity
                  ).toFixed(2)}
                </p>
              </div>
            </div>
